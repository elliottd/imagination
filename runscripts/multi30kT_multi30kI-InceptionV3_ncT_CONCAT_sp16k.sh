...
YAML=${HOME}/src/nmt/configs/multi30kT_multi30kI-InceptionV3_ncT_CONCAT_16ksp.yaml

COUNTER="$1" # the first argument to the bash script.

until [ $COUNTER = 0 ]; do

  OUTPUT_DIR=${MODEL_DIR}/multi30kT_multi30kI-InceptionV3_ncT_CONCAT_16ksp_${COUNTER}/
  mkdir -p ${OUTPUT_DIR}
